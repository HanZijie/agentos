...
```bash
./scripts/android.sh doctor       # 检查 SDK、Java、AVD 和 ADB 连接
./scripts/android.sh start        # 启动或复用 Pixel_8a，等待 Android 启动完成
./scripts/android.sh run          # 编译 debug APK、覆盖安装并重新启动 App
./scripts/android.sh configure-model # App 已安装时单独导入 .env.anthropic.local
./scripts/android.sh logs         # 当前 App 进程的实时日志，Ctrl+C 退出
./scripts/android.sh logs -d       # 读取当前 App 进程已有日志后退出
./scripts/android.sh crashes      # 读取设备的崩溃缓冲区
./scripts/android.sh screenshot   # 保存 build/codex/screenshot.png
./scripts/android.sh ui           # 保存 build/codex/ui.xml
```

`run` 安装的是 debug 构建，保留已有应用数据，并重新启动应用进程。默认目标是 `Pixel_8a`；脚本按 AVD 名称识别设备，不依赖 `emulator-5554` 这个可能变化的编号。

`.env.anthropic.local` 只在本机读取。设置 `ANTHROPIC_API_KEY`、`ANTHROPIC_MODEL`，可选设置 `ANTHROPIC_BASE_URL` 和 `ANTHROPIC_MAX_TOKENS`；脚本通过临时 ADB 文件交给 debug test runner，再写入 Android Keystore。密钥不会作为 Gradle 参数、APK 资源或日志输出。也可以使用 `ANTHROPIC_ENV_FILE=/path/to/file ./scripts/android.sh run` 指定配置文件。
...
```bash
./scripts/android.sh adb shell input tap 200 400
./scripts/android.sh adb shell input keyevent KEYCODE_BACK
./scripts/android.sh adb shell dumpsys activity activities
...
./scripts/android.sh instrumentation  # 自动安装 Notes Plugin，运行设备测试并导入本地 Anthropic 配置
```
...
```bash
ANDROID_AVD=Another_AVD ./scripts/android.sh run
ANDROID_SERIAL=emulator-5556 ./scripts/android.sh logs
```
...
Codex 的本地环境支持把项目命令设为顶部动作，动作会在内置终端中运行。可使用 `./scripts/android.sh run` 作为运行命令，使用 `./scripts/android.sh logs` 作为日志命令。参见 [OpenAI 本地环境文档](https://learn.chatgpt.com/docs/environments/local-environment)。
