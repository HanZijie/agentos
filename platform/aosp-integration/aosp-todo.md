...
当前基线：`platform/checkout` 尚未放置 AOSP checkout，下列全部条目为未开始。

## 0. 前置：checkout 与构建环境

- [ ] 选定目标 AOSP 版本与分支（Cuttlefish 可用、内核带 binder freezer 支持），记录版本号到本节。
...
- [ ] init rc：`system/agent/init/` 产出 `sideagentd.rc`（class、user/group、capabilities、重启策略），集成到产品 makefile。
...
- [ ] Binder 服务注册：sideagentd 注册稳定服务名，AgentManagerService 可发现并监管健康状态。

## 2. SELinux

- [ ] `sideagentd` domain：`system/agent/sepolicy/` 产出 domain、`file_contexts`、`service_contexts`，接入 `system/sepolicy` 或 vendor policy 目录。
...
- [ ] 在 `frameworks/base/services/` 新增 AgentManagerService，`SystemServer` 注册；只做控制面（生命周期、校验、路由），不跑 Agent loop。
...
- [ ] 按需 `bindServiceAsUser` + `openPluginSession` 握手，capability session 移交 sideagentd（契约 §4、§5）。
