...
AOSP 的第一阶段 overlay 位于 [`platform/aosp-integration/overlay/`](../../platform/aosp-integration/overlay/)。它目前只提供健康 Binder、init/SELinux 基线和 Plugin manifest discovery/按需 bind 骨架；必须在选定的 AOSP/Cuttlefish 分支上编译、启动并完成 freezer 矩阵后，才能称为系统实现。
